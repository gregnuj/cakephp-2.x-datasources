 ...
 * @subpackage    cake.cake.libs.model.datasources.dbo
*/
class OracleOci extends DboSource {
	...
	*/
	...
	public function connect() {
		...
		if (!$config['persistent']) {
			$this->_connection = oci_connect($config['login'], $config['password'], $config['database'], $config['charset']);
		} else {
			$this->_connection = oci_pconnect($config['login'], $config['password'], $config['database'], $config['charset']);
		}
	...
	public function describe($model) {
		...
		while ($result = oci_fetch_assoc($results)) {
	...
	public function disconnect() {
		if ($this->_connection) {
			$this->connected = !oci_close($this->_connection);
			return !$this->connected;
		}
	}
	...
	public function fetchResult() {
		...
		if ($row = oci_fetch_row($this->_result)) {
			$this->_cursorPosition++;
			$resultRow = array();
	...
	protected function _execute($sql, $params = array(), $prepareOptions = array()) {
		...
		if (!$query) {
			$this->_setError($this->_connection);
			return false;
		}

		foreach ($params as $pname -> $param){
			oci_bind_by_name($query, $pname, $param);
		}

		if ($this->_limit > 0) {
			oci_set_prefetch($query, $this->_limit);
		} else {
			oci_set_prefetch($query, $this->config['prefetch']);
		}
		...
		if (!oci_execute($query, $mode)) {
			$this->_result = $query;
			oci_free_statement($query);
			return false;
		}

		if (!oci_num_fields($query)) {
	...
	*/
	...
	public function getVersion() {
		return oci_server_version($this->__connection);
	}
	...
	/**
	 *
	 * The functions below here are carried over from 1.3 datasource
	 * They are not tested in 2.4 and not involved in standard CRUD operations
	 *
	 */
 ...
 */
	...
	/**
	 * Function below here are not in use
	 * I have not examined them closely to see if they provide any
	 * additional functionality than the ones in DboSource
	 *
	 */
	 ...
	 * @return string
	 */
	public function __renderStatement($type, $data) {
		extract($data);
		$aliases = null;
	 ...
	 * @param array $stack
	 */
	public function __queryAssociation(&$model, &$linkModel, $type, $association, $assocData, &$queryData, $external = false, &$resultSet, $recursive, $stack) {
